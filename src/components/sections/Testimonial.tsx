...
const Testimonial = () => {
  ...
  <Container>
    ...
    <div className='flex justify-between lg:items-center flex-col md:flex-row gap-2'>
            <h1 className='dark:text-white text-3xl md:text-4xl font-medium flex-1'>What Says From Our Happy Customers</h1>
        ...
        <div className='mt-10'>
            <Marquee className='rounded-md' pauseOnHover={true} gradientWidth={md?100:sm?50:200} gradientColor={'rgba(24,67,200,.5)'} gradient={true}>
                {
                testimonialReviews.map((review,index)=>(
                    <div key={index} className=' flex flex-col justify-between dark:bg-dark_light_color p-5 rounded-md  space-y-2  mr-10 min-h-72 shadow' style={{width:`${divide}px`}}>
                     <p className='dark:text-dark_text_high_color '>
                        {review.review_text}
                     </p>
                     <div className='flex gap-2'>
                        <img src={review.image} alt="img"  className='size-10 rounded-full'/>
                        <div className='space-y-1'>
                            <h3 className='text-xl font-medium dark:text-dark_text_high_color'>{review.name}</h3>
                            <h6 className='dark:text-dark_text_low_color'>{review.profession||'N/A'}</h6>
                        </div>
                     </div>
                    </div>
                ))
